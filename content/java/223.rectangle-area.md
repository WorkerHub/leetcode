...
2. **负数处理**：重叠宽度/高度可能为负，需要用 `Math.max(0, ...)`
3. **边界情况**：两个矩形刚好相邻（边重合但不重叠）
4. **坐标混淆**：左下角和右上角的坐标关系
5. **整数溢出**：坐标范围为 10^4，面积最大约 4×10^8，在 int 范围内；但在其他类似题目中需注意溢出，可改用 long
...
4. **代码简洁性**：学会使用 Math.max/Math.min 简化条件判断
5. **几何直觉**：多画图理解，培养几何直觉
